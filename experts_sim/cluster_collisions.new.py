...
for iter in range(args.num_iters):
    ...
    for clust_id in range(args.num_clusters):
        ...
        samples = clust_vecs[clust_id] + sample_noise
        ...
        assert samples_clust_noised_ids.size == args.sample_size
        sample_swaps_per_set.append((samples_clust_noised_ids != clust_id).sum())
        del samples_clust_noised_ids

    sample_swaps.append(np.array(sample_swaps_per_set).sum())
...
print(f'Swapped IDs: {args.data_dimension} {args.num_clusters} {args.noise_level} {sample_swaps} {total_samples} {1.0 - sample_swaps / total_samples} {std_swaps / sample_swaps}')
